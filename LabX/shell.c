...
int main(int argc, char *argv[]) {
   ...
   while (1) {
      ...
      }else if(command.name[0] == 'H')
      {
        char *newC = "more";
        command.name = newC;
        char *fileName = "manual";
        command.argv[1] = fileName;
        command.argv[2] = NULL;
      }
      else if(command.name[0] == 'X')
      {
      	char *newC = command.argv[1];
      	command.name = newC;
      	command.argv[0] = newC;
      	command.argv[1] == NULL;
      }
...
void printPrompt() {
    ...
    */
  const char *promptString = "linux pkm16|>";
